...
class PdfToText:
    ...
    "Poste : [Intitulé du poste] au sein du service [Nom du service]\n"
    "Tâches principales : [Liste des missions réalisées]\n"
    "Responsabilités : [Liste des responsabilités ou objectifs atteints]\n"
    "Résultats : [Impact de l'expérience, compétences développées, réussites marquantes]"
)
    ...
    def extract_text_from_pdf(self):
     """Convertir en scanné, extraire par OCR, puis compléter par pdfplumber si nécessaire."""
     try:
        # Étape 1 : Convertir PDF en images (simulateur de scan)
        pages = convert_from_path(self.pdf_path, dpi=300)

        # Étape 2 : OCR avec Tesseract sur chaque image
        tesseract_text = ""
        for page in pages:
            tesseract_text += pytesseract.image_to_string(page, config='--psm 4 --oem 3', lang='eng+fra') + "\n"

     except Exception as e:
        print(f"Erreur OCR : {e}")
        tesseract_text = ""

    # Étape 3 : Extraction texte brut avec pdfplumber
     try:
        with pdfplumber.open(self.pdf_path) as pdf:
            pdfplumber_text = "".join(page.extract_text() or "" for page in pdf.pages)
     except Exception as e:
    ...
    # Étape 4 : Comparer et ajouter les lignes manquantes
     tesseract_lines = set(line.strip() for line in tesseract_text.splitlines() if line.strip())
     additional_lines = []
     ...
     if additional_lines:
        print(f"{len(additional_lines)} lignes manquantes ajoutées à la fin du texte OCR.")
        tesseract_text += "\n" + "\n".join(additional_lines)

    # Étape 5 : Correction grammaticale (facultative)
     try:
        language = self.detect_language(tesseract_text)
        if language in ["en", "fr"]:
            blob = TextBlob(tesseract_text)
...
def main():
    pdf_path = r"C:\Users\OUMAIMA\Downloads\Rahma Saffar - PHP.pdf"
    offer = {
